...
/*
	...
	enum {
	  ...
	  kVK_Home                      = 0x73,
	  kVK_PageUp                    = 0x74,
	  fuckingmac_Delete             = 0x75,
	  kVK_F4                        = 0x76,
	  kVK_End                       = 0x77,
	  kVK_F2                        = 0x78,
	  kVK_PageDown                  = 0x79,
	  kVK_F1                        = 0x7A,
	  fuckingmac_Left                 = 0x7B,
	  fuckingmac_Right                = 0x7C,
	  fuckingmac_Down                 = 0x7D,
	  fuckingmac_Up                   = 0x7E
	};
...
for l in import:gmatch("([^\n]*)") do
	...
	for w in l:gmatch("([^%s]+)") do
		if define then
			value=w
			local n=tonumber(value:sub(3,4),16)
			if n then
				keynames[n]=string.lower(string.match(define,"[^_]+$"))
			end
			define=nil
		end
		if w=="=" then define=last end
		last=w
	end
end
...
osx.msg=function(w)
	...
	if m and m.class=="key" then
		m.keyname=keynames[m.keycode] or string.format("osx_%02x",m.keycode)

--print(m.keycode,m.keyname)
