...
local vendor_id = tonumber(arg[2])
local product_id = tonumber(arg[3])
if not vendor_id or not product_id or
   AND(vendor_id , 0x0000ffff) ~= vendor_id or AND(product_id , 0x0000ffff) ~= product_id then
...
--   local fmt = string.rep("B", #bytes)
   local t = wpack.load_array(bytes,"u8")
...
local function bcd2string(x)
   return string.format("%d%d.%d%d", AND(RSHIFT(x,12),0x0f), AND(RSHIFT(x,8),0x0f), AND(RSHIFT(x,4),0xf), AND(x,0xf))
end
