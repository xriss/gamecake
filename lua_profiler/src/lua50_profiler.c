...
//bug, maybe luajit diference or 5.1?
  lua_pop(L,1); // anyhow need to pop otherwise crap ends up on the stack?
...
int luaopen_profiler(lua_State *L) {
  lua_newtable(L); // lets not set any globals, mkay.
  luaL_openlib(L, NULL, prof_funcs, 0);
