...
-- build a bitmap which is a 256x1 array of characters using above fontdata
-- might grow to 256x2 or more in the future as we add more unicode characters
...
M.build_grd=function(hx,hy,style)
	...
	if not M.grds[name] then -- render
		...
		local g=wgrd.create("U8_RGBA_PREMULT",256*hx,1*hy,1)
		
		for i=0,255 do
			local s=data[i]
			if s then
				bitdown.pix_grd(s,bitdown.cmap_swanky32,g,(i%256)*hx,math.floor(i/256)*hy,hx,hy)
			end
		end
